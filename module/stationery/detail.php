    ...
    <div class="product_detail-left">
        <div class="pic-product_detail">
            <img class="img-thumbnail p-4" style="width: 330px;"
                src="<?= _HOST_ASSETS . '/images/product/' . $sanpham_detail['image'] ?>" alt="Ảnh sách" />
        </div>
        <div class="product_detail-right">
            <div class="name-profuct_detail">
                <?= $sanpham_detail['product_name'] ?>
            </div>
            <div class="tacgia-product_detail">
                Thương hiệu: <span><?= $sanpham_detail['brand_name'] ?></span>
            </div>
            <div class="tacgia-product_detail">
                Xuất xứ: <span><?= $sanpham_detail['country_name'] ?></span>
            </div>
            <div class="price-product_detail">
                Giá: <span><?= number_format($sanpham_detail['discount']) ?>đ</span>
            </div>
            <div class="price_sale-product_detail">
                <!-- <p> Tiết kiệm: <span>20,000 đ (10%)</span></p> -->
                <p> Tiết kiệm:
                    <span><?= number_format($sanpham_detail['price'] - $sanpham_detail['discount']) ?>đ</span>
                </p>
                <p>Giá thị trường: <span><?= number_format($sanpham_detail['price']) ?>đ</span></p>
                <p>Tồn kho: <span><?= $sanpham_detail['stock_quantity'] ?></span></p>
            </div>
            <form method="post">
                    ...
                    <div class="attr-content-pro-detail d-flex flex-wrap align-items-center justify-content-between">
                        <div class="quantity-pro-detail">
                            ...
                            <input name="quantity" type="number" class="qty-pro" min="1"
                                max="<?= $sanpham_detail['stock_quantity'] ?>" value="1" />
                            <span class="quantity-plus-pro-detail">+</span>
                        </div>
                    </div>
                </div>
                <input type="hidden" name="id" value="<?= $sanpham_detail['id'] ?>">
                <input type="hidden" name="title" value="<?= $sanpham_detail['product_name'] ?>">
                <input type="hidden" name="image" value="<?= $sanpham_detail['image'] ?>">
                <input type="hidden" name="price" value="<?= $sanpham_detail['price'] ?>">
                <input type="hidden" name="discount" value="<?= $sanpham_detail['discount'] ?>">
